...
@implementation BNHTMLNode

-(BNHTMLNode*)parent
{
	return [[BNHTMLNode alloc] initWithXMLNode:_node->parent];	
}

-(BNHTMLNode*)nextSibling {
	return [[BNHTMLNode alloc] initWithXMLNode:_node->next];
}

-(BNHTMLNode*)previousSibling {
	return [[BNHTMLNode alloc] initWithXMLNode:_node->prev];
}
...
-(BNHTMLNode*)firstChild
{
	return [[BNHTMLNode alloc] initWithXMLNode:_node->children];
}
		...
		for(xmlAttrPtr attr = cur_node->properties; NULL != attr; attr = attr->next)
		{
			...
			{				
				for(xmlNode * child = attr->children; NULL != child; child = child->next)
				{
					...
					{
						//Found node
						BNHTMLNode * nNode = [[BNHTMLNode alloc] initWithXMLNode:cur_node];
...
-(void)findChildTags:(NSString*)tagName inXMLNode:(xmlNode *)node inArray:(NSMutableArray*)array
{
	...
	{				
		...
		{
			BNHTMLNode * node = [[BNHTMLNode alloc] initWithXMLNode:cur_node];
...
-(BNHTMLNode*)findChildTag:(NSString*)tagName inXMLNode:(xmlNode *)node
{
	xmlNode *cur_node = NULL;
	const char * tagNameStr =  [tagName UTF8String];
	...
	{				
		...
		{
			return [[BNHTMLNode alloc] initWithXMLNode:cur_node];
		}
		
		BNHTMLNode * cNode = [self findChildTag:tagName inXMLNode:cur_node->children];
		if (cNode != NULL)
		{
			return cNode;
		}
...
-(BNHTMLNode*)findChildTag:(NSString*)tagName
{
...
-(BNHTMLNode*)findChildWithAttribute:(const char*)attribute matchingName:(const char*)name inXMLNode:(xmlNode *)node allowPartial:(BOOL)partial
{
	xmlNode *cur_node = NULL;
	...
	{		
		...
		BNHTMLNode * cNode = [self findChildWithAttribute:attribute matchingName:name inXMLNode:cur_node->children allowPartial:partial];
		if (cNode != NULL)
		{
			return cNode;
		}
...
-(BNHTMLNode*)findChildWithAttribute:(NSString*)attribute matchingName:(NSString*)className allowPartial:(BOOL)partial
{
...
-(BNHTMLNode*)findChildOfClass:(NSString*)className
{	
	BNHTMLNode * node = [self findChildWithAttribute:"class" matchingName:[className UTF8String]  inXMLNode:_node->children allowPartial:NO];
	return node;
}
